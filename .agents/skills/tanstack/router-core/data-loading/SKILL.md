...
- **Built-in SWR cache vs external cache coordination**: Router has built-in caching. When using TanStack Query, set `defaultPreloadStaleTime: 0` to avoid double-caching. See **react-router/router-query/SKILL.md**.
...
- See also: **react-router/router-query/SKILL.md** — for external cache coordination with TanStack Query
