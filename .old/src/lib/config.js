...
function validateConfig() {
  const activeTab = document
    .querySelector(".config-tab.active")
  ...
  if (activeTab === "standard") {
    ...
    const changes = {
      avatar1:
        document.getElementById("avatar1-name").textContent !==
        defaultValues.avatar1,
      avatar2:
        document.getElementById("avatar2-name").textContent !==
        defaultValues.avatar2,
      player1Name:
        document.getElementById("player1Name").value.trim() !==
        defaultValues.player1Name,
      player2Name:
        document.getElementById("player2Name").value.trim() !==
        defaultValues.player2Name,
      roundDuration:
      ...
      maxHealth:
        document.getElementById("maxHealth").value !== defaultValues.maxHealth,
    };

    // Enable OK button if at least one change has been made
    const changesCount = Object.values(changes).filter(
      (changed) => changed
    ).length;
    ...
    const isAdvancedValid =
      groupSelect.value !== "" && matchSelect.value !== "";
...
function saveConfig() {
  ...
  if (activeTab === "standard") {
    ...
    const roundDuration =
      parseInt(document.getElementById("roundDuration").value) * 1000;
    const breakDuration =
      parseInt(document.getElementById("breakDuration").value) * 1000;
    ...
    document.getElementById("redPlayer").textContent =
      document.getElementById("player1Name").value;
    document.getElementById("bluePlayer").textContent =
      document.getElementById("player2Name").value;
  } else {
    // Save advanced tab data
    // Get values from advanced tab
    const roundDuration =
      parseInt(document.getElementById("advancedRoundDuration").value) * 1000;
    const breakDuration =
      parseInt(document.getElementById("advancedBreakDuration").value) * 1000;
    const maxHealth = parseInt(
      document.getElementById("advancedMaxHealth").value
    );
    ...
    document.getElementById("redPlayer").textContent =
      document.getElementById("advanced-red-name").value;
    document.getElementById("bluePlayer").textContent =
      document.getElementById("advanced-blue-name").value;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("roundDuration")
    .addEventListener("input", validateConfig);
  document
    .getElementById("breakDuration")
    .addEventListener("input", validateConfig);
  document
    .getElementById("maxHealth")
    .addEventListener("input", validateConfig);
  document
  ...
  document.getElementById("avatar1").addEventListener("change", function (e) {
    ...
    if (e.target.files[0]) {
      ...
      const nameFromFile = e.target.files[0].name
        .split(".")
        .slice(0, -1)
        .join(".");
  ...
  document.querySelector(".menuButton").addEventListener("click", () => {
    ...
    const configSectionsContainer = document.querySelector(
  ...
  async function fetchMatchDetails(matchId) {
    try {
      ...
      if (match.matchNo) {
        const matchIdDisplay = document.querySelector(".matchId");
        if (matchIdDisplay) {
          matchIdDisplay.textContent = match.matchNo
            .toString()
            .padStart(3, "0");
        }
      }
