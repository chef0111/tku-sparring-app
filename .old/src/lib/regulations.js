...
function subtractHealth(player, healthPoints, event) {
  ...
  const currentScore = gameState.getState(
    player === 'red' ? 'redScore' : 'blueScore'
  );
  const currentHits = gameState.getState(
    player === 'red' ? 'redHits' : 'blueHits'
  );
  ...
  hitsElement.textContent = gameState.getState(
    player === 'red' ? 'redHits' : 'blueHits'
  );
...
function declareWinner() {
  const redHealth =
    gameState.getState('redHealth') || gameState.getState('maxHealth');
  const blueHealth =
    gameState.getState('blueHealth') || gameState.getState('maxHealth');
...
function resetRound() {
  ...
  if (!isBreakTime) {
    ...
    document.getElementById('redDelayedHP').style.width =
      `${healthPercentage}%`;
    document.getElementById('blueDelayedHP').style.width =
      `${healthPercentage}%`;
