...
function resetTimer() {
  ...
  const configuredRoundDuration =
    gameState.getState('configuredRoundDuration') || 60 * 1000;
  gameState.setState('timeLeft', configuredRoundDuration);
  gameState.setState(
    'breakTimeLeft',
    gameState.getState('breakTimeLeft') || 30 * 1000
  );
  gameState.setState('timerRunning', false);
...
function updateBreakTimer() {
  ...
  if (breakTimeLeft > 0 && breakTimerRunning) {
    ...
    if (currentRound < maxRounds) {
      ...
      const configuredRoundDuration = gameState.getState(
        'configuredRoundDuration'
      );
      gameState.setState('timeLeft', configuredRoundDuration);
      gameState.setState('timerRunning', false);
...
function hideRecord() {
  document.querySelector('.redScoreBox .recordSection').style.visibility =
    'hidden';
  document.querySelector('.blueScoreBox .recordSection').style.visibility =
    'hidden';
  document.querySelector('.redScoreBox .totalWins').style.visibility = 'hidden';
  document.querySelector('.blueScoreBox .totalWins').style.visibility =
    'hidden';
}
...
function showRecord() {
  ...
  document.querySelector('.blueScoreBox .recordSection').style.visibility =
    'visible';
  document.querySelector('.redScoreBox .totalWins').style.visibility =
    'visible';
  document.querySelector('.blueScoreBox .totalWins').style.visibility =
    'visible';
}
...
function updateWinner() {
  ...
  for (let i = 1; i <= currentRound; i++) {
    document.getElementById(`redWin${i}`).style.visibility =
      roundWinners[i - 1] === 'red' ? 'visible' : 'hidden';
    document.getElementById(`blueWin${i}`).style.visibility =
      roundWinners[i - 1] === 'blue' ? 'visible' : 'hidden';
  }
}
